...
Usage:
    python generate_tools_batch.py --count 200 --provider openai --api-key sk-xxx
    python generate_tools_batch.py --count 500 --provider deepseek --api-key sk-xxx --model deepseek-chat
    python generate_tools_batch.py --count 100 --resume  # resume from last checkpoint

The script:
1. Loads existing tools from tools/ as the reference pool
...
5. Saves to tools/ and adds to the growing pool
...
def load_existing_tools(tools_dir: Path) -> list[dict]:
    """Load TOOL_SPEC from all *.py files in tools/ (excluding __init__.py)."""
    tools: list[dict] = []
...
def _extract_tool_spec(path: Path) -> dict | None:
    """Extract TOOL_SPEC dict from a Python module without executing it."""
    try:
    ...
    if start == -1:
        return None
    # Extract the dict by tracking braces
...
def write_tool_module(tool_data: dict, tools_dir: Path) -> Path:
    """Write a generated tool dict to a Python module file in tools/."""

...
# Common beta headers for enhanced features.
_COMMON_BETAS = [
...
def _resolve_anthropic_token() -> str | None:
    """Resolve Anthropic credential from environment variables.
...
def _build_async_anthropic_client_kwargs(
    api_key_or_token: str,
    base_url: str | None = None,
) -> dict[str, Any]:
    """Keyword arguments for ``AsyncAnthropic`` client construction."""
    ...
    if _is_oauth_token(api_key_or_token):
...
class AnthropicProvider(BaseProvider):
    ...
    def client(self) -> AsyncAnthropic:
        ...
        if self._client is None:
            ...
            if not raw:
                raw = (_resolve_anthropic_token() or "").strip()
            if not raw:
                raise ValueError(
                    "Anthropic credentials are required. "
                    "Set ANTHROPIC_API_KEY, ANTHROPIC_TOKEN, "
                    "or pass api_key in settings."
                )

            client_kwargs = _build_async_anthropic_client_kwargs(raw, self.base_url)

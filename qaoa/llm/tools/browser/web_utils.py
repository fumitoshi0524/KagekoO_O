#!/usr/bin/env python3
# QAOA category: search, domain: technology
"""
Standalone Web Tools Module

This module provides generic web tools that work with multiple backend providers.
Backend is selected via Kageko Settings (``kageko.web.backend``).
...
from .auxiliary_llm import async_call_llm, check_auxiliary_model as _check_aux_model_available
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from Kageko Settings."""
    try:
        return get_settings().web.model_dump()
    except Exception:
        return {}
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from Kageko Settings.
    Falls back to whichever API key is present for users who configured
    keys manually without running setup.
    """
    configured = _load_web_config().get("backend", "").lower().strip()
    if configured == "firecrawl" and Firecrawl is None:
        # Keep backend selection downgrade-friendly when optional deps
        # are not installed.
        configured = ""
...
def _get_firecrawl_client():
    ...
    if _firecrawl_client is None:
        if Firecrawl is None:
            raise ImportError(
                "Firecrawl backend is selected but `firecrawl-py` is not installed. "
...
async def process_content_with_llm(
    ...
    This function uses the configured auxiliary model via the provider router
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    with multiple backends (Parallel, Firecrawl, or Tavily).
...
async def web_extract_tool(
    ...
    Args:
        ...
        model (str): The model to use for LLM processing
    ...
    try:
        ...
        if backend == "parallel":
            ...
            from ..environments.interrupt import is_interrupted as _is_interrupted
            for url in urls:
                if _is_interrupted():
...
async def web_crawl_tool(
    ...
    try:
        ...
        if backend == "tavily":
            ...
            from ..environments.interrupt import is_interrupted as _is_int
            if _is_int():
                return json.dumps({"error": "Interrupted", "success": False})
...
def check_auxiliary_model() -> bool:
    ...
    try:
        return bool(
            _check_aux_model_available(model=DEFAULT_SUMMARIZER_MODEL)
        )
    except Exception:
        return False
...
if __name__ == "__main__":
    ...
    """
    print("\U0001f310 Standalone Web Tools Module")
    print("=" * 40)
    ...
    print("\U0001f6e0️  Web tools ready for use!")

    if nous_available:
        print(f"\U0001f9e0 LLM content processing available with {DEFAULT_SUMMARIZER_MODEL}")
        print(f"   Default min length for processing: {DEFAULT_MIN_LENGTH_FOR_SUMMARIZATION} chars")

    # Show debug mode status
    if _debug.active:
        print(f"\U0001f41b Debug mode ENABLED - Session ID: {_debug.session_id}")
        print(f"   Debug logs will be saved to: {_debug.log_dir}/web_tools_debug_{_debug.session_id}.json")
    else:
        print("\U0001f41b Debug mode disabled (set WEB_TOOLS_DEBUG=true to enable)")
    ...
    print(f"\n\U0001f4dd Run 'python test_web_tools_llm.py' to test LLM processing capabilities")

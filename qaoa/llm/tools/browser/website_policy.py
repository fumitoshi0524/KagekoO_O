...
This module loads a user-managed website blocklist from Kageko Settings
(``settings.website_blocklist``) and optional shared list files.
It is intentionally lightweight so web/browser
...
def _get_kageko_data_dir() -> Path:
    """Resolve Kageko data directory for relative shared_files.

    When settings aren't available (e.g. imported early), fall back to
    ``$KAAGEKO_HOME`` or ``~/.kageko``.
    """

    try:
        return get_settings().get_data_directory()
    except Exception:
        return Path(os.getenv("KAAGEKO_HOME", Path.home() / ".kageko"))


def _get_default_config_path() -> Path:
    return _get_kageko_data_dir() / "config.yaml"
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    resolved_path = str(config_path) if config_path else "__settings__"
    now = time.monotonic()

    # Return cached policy if still fresh (settings-based only)
    if config_path is None:
        ...
        # Settings-based (Kageko-local mode): no YAML read.
        try:
            settings = get_settings()
            enabled = bool(settings.website_blocklist.enabled)
            raw_domains = settings.website_blocklist.domains or []
            raw_shared_files = settings.website_blocklist.shared_files or []
        except Exception:
            enabled = bool(_DEFAULT_WEBSITE_BLOCKLIST.get("enabled", False))
            raw_domains = []
            raw_shared_files = []
    ...
    for shared_file in raw_shared_files:
        ...
        if not path.is_absolute():
            path = (_get_kageko_data_dir() / path).resolve()
    ...
    # Cache the result for settings-based mode only.
    if config_path is None:
        with _cache_lock:
            _cached_policy = result
            _cached_policy_path = "__settings__"

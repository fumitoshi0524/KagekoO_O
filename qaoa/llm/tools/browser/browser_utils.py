#!/usr/bin/env python3
# QAOA category: operations, domain: technology
"""
Browser Tool Module
...
from .auxiliary_llm import call_llm
from ....config.settings import get_settings

try:
    from .website_policy import check_website_access
except Exception:  # pragma: no cover
    # Fail-open if policy module isn't importable; the tool wrapper may decide
    # to enforce access rules at a higher level.
    check_website_access = lambda url: None  # noqa: E731

from .browser_providers.base import CloudBrowserProvider
from .browser_providers.browserbase import BrowserbaseProvider
from .browser_providers.browser_use import BrowserUseProvider
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    try:
        provider_key = getattr(get_settings().browser, "cloud_provider", None)
        if provider_key and provider_key in _PROVIDER_REGISTRY:
            _cached_cloud_provider = _PROVIDER_REGISTRY[str(provider_key)]()
    except Exception as e:
        logger.debug("Could not read cloud_provider from settings: %s", e)
    return _cached_cloud_provider
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-kageko_...`` the resulting socket path exceeds the
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes Kageko-managed Node first (if present), then
        # legacy-managed Node as a migration fallback, then system dirs.
        node_bin_candidates: list[str] = []
        try:
            data_dir = get_settings().get_data_directory()
            node_bin_candidates.append(str(Path(data_dir) / "node" / "bin"))
        except Exception:
            pass

        fallback_home = Path(os.environ.get("KAAGEKO_HOME", Path.home() / ".kageko"))
        fallback_node_bin = str(fallback_home / "node" / "bin")
        if os.path.isdir(fallback_node_bin):
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        record_enabled = os.getenv("BROWSER_RECORD_SESSIONS", "").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
        ...
        if config_path.exists():
            try:
                ...
                record_enabled = cfg.get("browser", {}).get(
                    "record_sessions", record_enabled
                )
            except Exception:
                # Recording is best-effort; don't break browser tool usage.
                pass

        if not record_enabled:
            return

        recordings_dir = Path(data_dir) / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    data_dir = get_settings().get_data_directory()
    screenshots_dir = Path(data_dir) / "browser_screenshots"
...
def _cleanup_old_recordings(max_age_hours=72):
    ...
    try:
        try:
            ...
            data_dir = Path(os.getenv("KAAGEKO_HOME", Path.home() / ".kageko"))
        recordings_dir = Path(data_dir) / "browser_recordings"
...
if __name__ == "__main__":
    ...
    """
    print("\U0001f310 Browser Tool Module")
    print("=" * 40)
    ...
    print("\n\U0001f4cb Available Browser Tools:")
    for schema in BROWSER_TOOL_SCHEMAS:
        print(f"  \U0001f539 {schema['name']}: {schema['description'][:60]}...")
...
# ---------------------------------------------------------------------------
try:
    # Legacy registry (optional). Kageko wires tools via BaseTool later.
    from tools.registry import registry  # type: ignore
except Exception:  # pragma: no cover
    class _NoOpRegistry:  # noqa: D401
        """No-op stand-in so this module remains importable."""

        def register(self, **_: Any) -> None:  # type: ignore[override]
            return None

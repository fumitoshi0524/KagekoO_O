"""execute_code tool -- Hermes-style entrypoint (Kageko-local implementation).

Parity with ClawCode -- same ExecuteCodeTool, create_execute_code_tool,
_run_shell_command, _run_python_sandbox.

Hermes provides programmatic tool calling (PTC) via `execute_code`.
In Kageko we keep the same *entrypoint capability*:
...
async def _run_python_sandbox(
    *,
    ...
    max_tool_calls = int(os.getenv("KAGEKO_EXEC_CODE_PTC_MAX_TOOL_CALLS", "50") or 50)
    tool_call_counter = 0
    ...
    async def _dispatch(tool_name: str, args: dict[str, Any], context: ToolContext) -> dict[str, Any]:
        ...
        if tool_name == "read_file":
            ...
            # Hermes read_file offset is 1-indexed lines; kageko view offset is 0-indexed.
            kageko_offset = max(0, offset - 1)
            resp = await view_tool.run(
                ToolCall(
                    id=call_id,
                    name="view",
                    input={"file_path": path, "offset": kageko_offset, "limit": limit},
                ),
                rpc_ctx,
            )

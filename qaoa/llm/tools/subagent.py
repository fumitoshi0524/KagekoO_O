"""Subagent support aligned with Claude Code (Agent / Task tool, .kageko/agents/).

Parity with ClawCode -- same SubAgent, SubAgentContext, SubAgentResult,
SubAgentType, SubAgentEventType, IsolationMode, AgentTool, SubagentRunFinal,
create_agent_tool, create_subagent_tool, filter_delegate_tools, etc.
"""
...
# Claude Code tool names -> Kageko internal tool names (None = unsupported / delegate)
CLAUDE_TO_KAGEKO_TOOL: dict[str, str | None] = {
    "Read": "view",
    "Write": "write",
    "Edit": "edit",
    "Glob": "glob",
...
def _claude_name_to_kageko(name: str) -> str | None:
    key = name.strip()
    if key in CLAUDE_TO_KAGEKO_TOOL:
        return CLAUDE_TO_KAGEKO_TOOL[key]
    kl = key.lower()
    if kl in CLAUDE_TO_KAGEKO_TOOL:
        return CLAUDE_TO_KAGEKO_TOOL[kl]
...
def normalize_claude_tool_list(names: list[str]) -> list[str]:
    out: list[str] = []
    for raw in names:
        mapped = _claude_name_to_kageko(raw)
        if mapped:
            out.append(mapped)
...
class SubAgentContext:
    ...
    #: Parent Kageko shared cap; inner :class:`~qaoa.llm.agent.Agent.run` consumes the same pool.
    ...
    async def run(self) -> AsyncIterator[Any]:
        ...
        try:
            ...
            if self._context.isolation_mode == IsolationMode.WORKTREE:
                base = Path(self._context.working_directory) / ".kageko-subagent-worktrees"
...
class AgentTool(BaseTool):
    ...
    def info(self) -> ToolInfo:
        return ToolInfo(
            ...
            description=(
                ...
                "or custom agents from .kageko/agents/. "
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "allowed_tools": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Override allowed tools (Kageko internal or Claude names).",
                    },
                },
                "required": [],
            },
    ...
    def _prepare_subagent_run(
        ...
        if not dfn:
            ...
            if candidates:
                # Prefer same family (e.g. kagekoteam-*) when available.

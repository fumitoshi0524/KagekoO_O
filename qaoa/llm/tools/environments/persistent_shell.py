...
"""Persistent shell mixin: file-based IPC for long-lived bash."""
...
def _shell_fs_path(path: str) -> str:
    """Paths embedded in bash scripts: forward slashes, Git-Bash-friendly on Windows."""
    return path.replace("\\", "/")


class PersistentShellMixin:
    """Mixin adding persistent shell capability. Subclasses implement shell hooks."""

    persistent: bool

    @abstractmethod
    def _spawn_shell_process(self) -> subprocess.Popen[Any]: ...
    ...
    def _temp_prefix(self) -> str:
        base = os.path.join(tempfile.gettempdir(), f"kageko-persistent-{self._session_id}")
        return _shell_fs_path(base)

    def _init_persistent_shell(self) -> None:
    ...
    def _cleanup_persistent_shell(self) -> None:
    ...
    def _drain_shell_output(self) -> None:
        try:
            if self._shell_proc and self._shell_proc.stdout:
    ...
    def _execute_persistent_locked(self, command: str, cwd: str, timeout: int) -> dict[str, str | int]:
...
__all__ = ["PersistentShellMixin", "_shell_fs_path"]

...
"""SSH remote execution with ControlMaster (reference ``SSHEnvironment`` aligned)."""
...
def _ensure_ssh_available() -> None:
    if not shutil.which("ssh"):
        raise RuntimeError(
            "SSH is not installed or not in PATH. Install an OpenSSH client."
        )


class SSHEnvironment(PersistentShellMixin, BaseEnvironment):
    """Run commands on a remote host over SSH (ControlMaster + optional persistent shell)."""
    ...
    def __init__(
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "kageko-ssh"
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/kageko-ssh-{self._session_id}"
    ...
    def _read_temp_files(self, *paths: str) -> list[str]:
        ...
        delim = f"__KAAGEKO_SEP_{self._session_id}__"
    ...
    def _execute_oneshot(
        ...
        *,
        ...
        rc = proc.returncode
        return {"output": "".join(_output_chunks), "returncode": int(rc if rc is not None else -1)}

...
def skill_import(
    ...
    for file_path in files:
        try:
            if fmt == "auto":
                # Detect format from path or content
            ...
            # Write as Kageko-native markdown skill
...
def main(
    ...
    session_id = session if session != "default" else None
    state = ctx.obj
    # Auto-generate a session ID if none provided
    if session_id is None:
        sessions = runtime.list_sessions()
        session_id = sessions[-1] if sessions else __import__('uuid').uuid4().hex[:6]
        # If "default" has messages, use that; otherwise create fresh
    ...
    console.print(Panel(
        ...
        f"{key_warning}\n\n"
        "Type [bold]/help[/bold] for commands, [bold]/exit[/bold] to quit. [dim]Tab to complete.[/dim]",
        border_style="cyan",
        box=box.ROUNDED,
    ))

    # REPL loop using ReplDispatcher with tab completion
    ...
    dispatcher = ReplDispatcher(
        ...
        workspace=str(Path(workspace or os.getenv("KAGEKO_WORKSPACE") or os.getcwd()).resolve()),
    )
    prompt_session = dispatcher.create_prompt_session()

    while dispatcher.running:
        line = repl_input(prompt_session)
        if line is None:
            console.print("\n[dim]Goodbye.[/dim]")
            break
        if not line:
        ...
        # Handle /setup specially (needs CliState access)
        if line == "/setup":
